    ...
    public function run()
    {
        $products = [
            ...
            [
                'name' => 'ピーチ',
                'price' => 1000,
                'description' => '豊潤な香りととろけるような甘さが魅力のピーチ。美味しさはもちろん見た目の可愛さも抜群の商品です。ビタミンEも豊富なため、生活習慣病の予防にもおすすめです。もぎたてフルーツのスムージーをお召し上がりください！',
        ...
        foreach ($products as $item) {
            ...
            if (!empty($item['seasons'])) {
                $seasonNames = explode('、', $item['seasons']); // ['春', '夏']など
                $seasonIds = Seasons::whereIn('name', $seasonNames)->pluck('id')->toArray();

                if (!empty($seasonIds)) {
                    $product->seasons()->attach($seasonIds);  // ID配列を渡す
                }
            }
        }
    }
